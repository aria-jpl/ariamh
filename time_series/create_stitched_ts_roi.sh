...
echo "##########################################" 1>&2
echo -n "Running stitched time-series generation: " 1>&2
date 1>&2
python3 $BASE_PATH/create_stitched_ts_roi.py _context.json > create_stitched_ts_roi.log 2>&1
STATUS=$?
echo -n "Finished running stitched time-series generation: " 1>&2
date 1>&2
if [ $STATUS -ne 0 ]; then
  echo "Failed to run stitched time-series generation." 1>&2
  cat create_stitched_ts_roi.log 1>&2
  echo "{}"
  exit $STATUS
fi
