...
def create_prov_es_json(ctx_file, prod_dir, prov_file):
    ...
    # get info
    s1_files = glob(os.path.join(prod_dir, "measurement", "s1%s-*.tiff" % mis_char_lc))
    pf = "S1%s" % mis_char
    dtype = "NA" 
    repo_dir = "?"
    if len(s1_files) > 0:
        match = S1_RE.search(os.path.basename(s1_files[0]))
        if match: swathnum, dtype = match.groups()
        if dtype == "raw":
            dtype = "RAW"
    ...
    output_ds = doc.granule(output_id, None, [prod_dir], [instrument],
                            ...
                            bundle=bndl)
    zip_output_id = "hysds:%s" % get_uuid(zip_url)
    zip_output_ds = input_ds
    ...
    doc.processStep("hysds:%s" % get_uuid(job_id), fake_time, fake_time, 
                    ...
                    [output_ds.identifier, zip_output_ds.identifier],
                    label=job_id, bundle=bndl, prov_type="hysds:ingest")
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("%s <context JSON> <product dir> <PROV-ES JSON file>" % sys.argv[0])
        sys.exit(1)

    create_prov_es_json(sys.argv[1], sys.argv[2], sys.argv[3])
    sys.exit(0)
