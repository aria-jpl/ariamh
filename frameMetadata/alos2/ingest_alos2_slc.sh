...
date 1>&2
python3 $BASE_PATH/ingest_alos2_slc.py > ingest_alos2_slc.log 2>&1
STATUS=$?
echo -n "Finished running ALOS2 interferogram generation: " 1>&2
date 1>&2
if [ $STATUS -ne 0 ]; then
  echo "Failed to run ALOS2 interferogram generation." 1>&2
  echo "# ----- errors|exception found in log -----" >> _alt_traceback.txt && grep -i "error\|exception" ingest_alos2_slc.log >> _alt_traceback.txt
  cat ingest_alos2_slc.log 1>&2
  echo "{}"
  exit $STATUS
fi
