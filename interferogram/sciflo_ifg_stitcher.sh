...
echo "##########################################" 1>&2
echo -n "Running interferogram stitcher sciflo: " 1>&2
date 1>&2
/usr/bin/python $BASE_PATH/sciflo_ifg_stitcher.py > sciflo_ifg_stitcher.log 2>&1
STATUS=$?
echo -n "Finished running interferogram stitcher sciflo: " 1>&2
date 1>&2
if [ $STATUS -ne 0 ]; then
  echo "Failed to run interferogram stitcher sciflo." 1>&2
