...
echo "##########################################" 1>&2
echo -n "Running validate_ts.py: " 1>&2
date 1>&2
python3 $BASE_PATH/validate_ts.py > validate_ts.log 2>&1
STATUS=$?
echo -n "Finished running validate_ts.py: " 1>&2
date 1>&2
if [ $STATUS -ne 0 ]; then
  echo "Failed to run validate_ts.py." 1>&2
  cat validate_ts.log 1>&2
  echo "{}"
  exit $STATUS
fi
