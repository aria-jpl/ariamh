...
class MergedTrackStitcher:
    ...
    def create_mask(self, sizes):
        ...
        if not os.path.exists(oname):
            ...
            bbox = ''.join(
                str([int(np.floor(minlat)), int(np.ceil(maxlat)), int(np.floor(minlon)), int(np.ceil(maxlon))]).split())
            uu = UrlUtils()
    ...
    def two_stage_unwrap(self, unwrappedIntFilename, ccFile, unwrapped2StageFilename=None, unwrapper_2stage_name=None,
                         solver_2stage=None):
