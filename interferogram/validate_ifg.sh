...
echo "##########################################" 1>&2
echo -n "Running validate_ifg.py: " 1>&2
date 1>&2
python3 $BASE_PATH/validate_ifg.py > validate_ifg.log 2>&1
STATUS=$?
echo -n "Finished running validate_ifg.py: " 1>&2
date 1>&2
if [ $STATUS -ne 0 ]; then
  echo "Failed to run validate_ifg.py." 1>&2
  cat validate_ifg.log 1>&2
  echo "{}"
  exit $STATUS
fi
