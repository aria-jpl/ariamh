...
date 1>&2
python3 $BASE_PATH/alos2_packaging.py > alos2_packaging.log 2>&1
STATUS=$?
echo -n "Finished running ALOS2 interferogram generation: " 1>&2
date 1>&2
if [ $STATUS -ne 0 ]; then
  echo "Failed to run ALOS2 interferogram generation." 1>&2
  echo "# ----- errors|exception found in log -----" >> _alt_traceback.txt && grep -i "error\|exception" alos2_packaging.log >> _alt_traceback.txt
  cat alos2_packaging.log 1>&2
  echo "{}"
  exit $STATUS
fi
