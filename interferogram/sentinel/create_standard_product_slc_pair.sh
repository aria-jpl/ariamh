...
export PYTHONPATH=.:$ISCE_HOME/applications:$ISCE_HOME/components:$BASE_PATH:$ARIAMH_HOME:$UTILS_HOME:$ARIAMH_HOME/interferogram:$TROPMAP_HOME:$PYTHONPATH
...
date 1>&2
python $BASE_PATH/create_standard_product_slc_pair.py > create_standard_product_slc_pair.log 2>&1
STATUS=$?
...
if [ $STATUS -ne 0 ]; then
  ...
  echo "# ----- errors|exception found in log -----" >> _alt_traceback.txt && grep -i "error\|exception" create_standard_product_slc_pair.log >> _alt_traceback.txt
  cat create_standard_product_slc_pair.log 1>&2
  echo "{}"
  exit $STATUS
fi
