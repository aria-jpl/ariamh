...
date 1>&2
python $BASE_PATH/create_standard_coseismic_product_s1.py > create_standard_coseismic_product_s1.log 2>&1
STATUS=$?
...
if [ $STATUS -ne 0 ]; then
  ...
  echo "# ----- errors|exception found in log -----" >> _alt_traceback.txt && grep -i "error\|exception" create_standard_coseismic_product_s1.log >> _alt_traceback.txt
  cat create_standard_coseismic_product_s1.log 1>&2
  echo "{}"
  exit $STATUS
fi
