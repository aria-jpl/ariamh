...
echo "##########################################" 1>&2
echo -n "Running topsApp registered SLC pair sciflo: " 1>&2
date 1>&2
/usr/bin/python $BASE_PATH/sciflo_topsapp_slcp.py > sciflo_topsapp_slcp.log 2>&1
STATUS=$?
echo -n "Finished running topsApp registered SLC pair sciflo: " 1>&2
date 1>&2
if [ $STATUS -ne 0 ]; then
  echo "Failed to run topsApp registered SLC pair sciflo." 1>&2
  cat sciflo_topsapp_slcp.log 1>&2
  echo "{}"
  exit $STATUS
fi
