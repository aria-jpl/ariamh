...
echo "##########################################" 1>&2
echo -n "Running dense offsets generation : " 1>&2
date 1>&2
python3 $BASE_PATH/create_dense_offsets.py > create_dense_offsets.log 2>&1
STATUS=$?
echo -n "Finished running dense offsets generation: " 1>&2
date 1>&2
if [ $STATUS -ne 0 ]; then
  echo "Failed to run dense offsets generation." 1>&2
  echo "# ----- errors|exception found in log -----" >> _alt_traceback.txt && grep -i "error\|exception" create_dense_offsets.log >> _alt_traceback.txt
  cat create_dense_offsets.log 1>&2
  echo "{}"
  exit $STATUS
fi
