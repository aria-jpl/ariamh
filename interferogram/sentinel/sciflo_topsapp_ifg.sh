...
echo "##########################################" 1>&2
echo -n "Running topsApp interferogram sciflo: " 1>&2
date 1>&2
/usr/bin/python $BASE_PATH/sciflo_topsapp_ifg.py > sciflo_topsapp_ifg.log 2>&1
STATUS=$?
echo -n "Finished running topsApp interferogram sciflo: " 1>&2
date 1>&2
if [ $STATUS -ne 0 ]; then
  echo "Failed to run topsApp interferogram sciflo." 1>&2
  cat sciflo_topsapp_ifg.log 1>&2
  echo "{}"
  exit $STATUS
fi
