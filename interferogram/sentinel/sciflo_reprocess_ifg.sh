...
echo "##########################################" 1>&2
echo -n "Running S1 interferogram reprocessing sciflo: " 1>&2
date 1>&2
/usr/bin/python $BASE_PATH/sciflo_reprocess_ifg.py > sciflo_reprocess_ifg.log 2>&1
STATUS=$?
echo -n "Finished running S1 interferogram reprocessing sciflo: " 1>&2
date 1>&2
if [ $STATUS -ne 0 ]; then
  echo "Failed to run S1 interferogram reprocessing sciflo." 1>&2
  cat sciflo_reprocess_ifg.log 1>&2
  echo "{}"
  exit $STATUS
fi
