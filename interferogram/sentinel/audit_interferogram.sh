...
echo "##########################################" 1>&2
echo -n "Running S1 audit interferogram: " 1>&2
date 1>&2
#Mimic sciflo by calling the program with the entrinsic python
$BASE_PATH/audit_interferogram.py > audit_interferogram.log 2>&1
STATUS=$?
echo -n "Finished running S1 audit interferogram sciflo: " 1>&2
date 1>&2
if [ $STATUS -ne 0 ]; then
  echo "Failed to run S1 audit interferogram sciflo." 1>&2
  cat audit_interferogram.log 1>&2
  echo "{}"
  exit $STATUS
fi
