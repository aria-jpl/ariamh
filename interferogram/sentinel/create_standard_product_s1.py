...
BASE_URL_KILAUEA = 'https://aria-alt-dav.jpl.nasa.gov/repository'
KILAUEA_DEM_XML = f'{BASE_URL_KILAUEA}/products/kilauea/dem_kilauea.dem.xml'
KILAUEA_DEM = f'{BASE_URL_KILAUEA}/products/kilauea/dem_kilauea.dem'
...
LOG_NAME = 'create_standard_product_s1.log'
DATASET_KEY = 'S1-GUNW'
...
def get_dataset_by_hash_version(ifg_hash, version, es_index='grq'):
    ...
    if '100.67.35.28' in es_url:
        es_url = es_url.replace(':9200', '')
...
def main():
    ...
    url = (f'{rest_url}/{uu.grq_index_prefix}'
           '/_search?search_type=scan&scroll=60&size=100')
    ...
    tiler_cmd_path = os.path.abspath(os.path.join(BASE_PATH,
                                                  '..',
                                                  '..',
                                                  'map_tiler'))
    ...
    check_call(tiler_cmd_tmpl.format(tiler_cmd_path,
                                     vrt_prod_file_dis,
                                     tiles_dir,
                                     dis_layer,
                                     -3.14,
    ...
    tiler_cmd_tmpl = ('{}/create_tiles.py {} {}/{} '
                      '-b 1 -m gray --clim_min {} --clim_max {} --nodata 0')
    ...
    check_call(extract_cmd_tmpl.format(extract_cmd_path,
