...
def checkBurstError():
    ...
    found, line = fileContainsMsg("create_dense_offsets.log", msg)
    if found:
...
def main():
    ...
    create_input_xml(os.path.join(BASE_PATH, 'topsApp_dense_offsets.xml.tmpl'), xml_file,
    ...
    topsapp_cmd = [
        "topsApp.py", "--steps", "--start=rangecoreg", "--end=geocodeoffsets",
    ]
    topsapp_cmd_line = " ".join(topsapp_cmd)
    logger.info("Calling topsApp.py to geocodeoffsets step: {}".format(topsapp_cmd_line))
...
def updateErrorFiles(msg):
    ...
    with open('_alt_traceback.txt', 'w') as f:
        '''
        with open("create_dense_offsets.log", 'r') as f2:
