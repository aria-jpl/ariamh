...
echo "##########################################" 1>&2
echo -n "Running S1 audit slc_pair product sciflo: " 1>&2
date 1>&2
/usr/bin/python $BASE_PATH/sciflo_audit_rsp.py > sciflo_audit_rsp.log 2>&1
STATUS=$?
echo -n "Finished running S1 audit slc_pair sciflo: " 1>&2
date 1>&2
if [ $STATUS -ne 0 ]; then
  echo "Failed to run S1 audit slc_pair sciflo." 1>&2
  cat sciflo_audit_rsp.log 1>&2
  echo "{}"
  exit $STATUS
fi
