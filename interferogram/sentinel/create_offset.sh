...
echo "##########################################" 1>&2
echo -n "Running S1 interferogram offset generation: " 1>&2
date 1>&2
python3 $BASE_PATH/create_offset.py > create_offset.log 2>&1
STATUS=$?
echo -n "Finished running S1 interferogram offset generation: " 1>&2
date 1>&2
if [ $STATUS -ne 0 ]; then
  echo "Failed to run S1 interferogram generation." 1>&2
  cat create_offset.log 1>&2
  echo "{}"
  exit $STATUS
fi
