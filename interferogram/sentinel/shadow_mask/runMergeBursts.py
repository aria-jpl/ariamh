...
def runMergeBursts(self, adjust=1):
    ...
    ### START lines for layover ###
    mergeBursts2(frames, os.path.join(self._insar.geometryDirname, 'IW%d', 'shadowMask_%02d.rdr'), burstIndex, box, os.path.join(mergedir, 'shadowMask.rdr'+suffix), virtual=virtual, validOnly=validOnly)
    mergeBursts2(frames, os.path.join(self._insar.geometryDirname, 'IW%d', 'incLocal_%02d.rdr'), burstIndex, box, os.path.join(mergedir, 'incLocal.rdr'+suffix), virtual=virtual, validOnly=validOnly)
    cmd = "gdal_translate -of ENVI {} {}".format(os.path.join(mergedir, 'lat.rdr'+suffix+'.vrt'), os.path.join(mergedir, 'lat.rdr'+suffix))
    runCmd(cmd)
    cmd = "gdal_translate -of ENVI {} {}".format(os.path.join(mergedir, 'lon.rdr'+suffix+'.vrt'), os.path.join(mergedir, 'lon.rdr'+suffix))
    runCmd(cmd)
    cmd = "gdal_translate -of ENVI {} {}".format(os.path.join(mergedir, 'shadowMask.rdr'+suffix+'.vrt'), os.path.join(mergedir, 'shadowMask.rdr'+suffix))
    runCmd(cmd)
    cmd = "gdal_translate -of ENVI {} {}".format(os.path.join(mergedir, 'incLocal.rdr'+suffix+'.vrt'), os.path.join(mergedir, 'incLocal.rdr'+suffix))
    runCmd(cmd)
    ### END lines for layover ###
