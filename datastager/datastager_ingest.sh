...
# ------------------------------------------------------------------------------
# for now, only interested in CSK RAW files

# want:   CSKS4_RAW_B_HI_03_HH_RA_SF_20130713133755_20130713133802.h5
# ignore: CSKS3_SCS_B_S2_20_HH_RD_SF_20120514010036_20120514010044.h5

# check if have RAW h5 file
...
# ------------------------------------------------------------------------------
# run ingest
fab -f $HOME/verdi/ops/product_ingest/product_ingest.py \
ingest:$INCOMING_HOST,$DATASETS_CFG,$GRQ_UPDATE_URL,$PRODUCT_PROCESSED_QUEUE,$PROD_DIR
ingest_status=$?

# handle ingest status
if [ "$ingest_status" -eq 0 ]; then
  #cleanup to prevent job worker from staging
  rm -rf $PROD_DIR *.h5 *.tar.gz *.xml
fi
exit $ingest_status
