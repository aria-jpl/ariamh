...
#    shutil.copyfile(DEMfile, os.path.join(inps.prepDir, 'hgt.flt'))
    ...
    #######Create userfn.py
    rdict = {}
    rdict['relpath'] = os.path.relpath(inps.srcDir, inps.prepDir)
    rdict['polyorder'] = 1
    ...
    rdict['cohth'] = 0.2
    rdict['nvalid'] = int(0.5 * len(pairs))
    latlon = np.loadtxt(inps.ref)
    rdict['rx0'] = latlon[1]-5
    rdict['rx1'] = latlon[1]+5
    rdict['ry0'] = latlon[0]-5
    rdict['ry1'] = latlon[0]+5
    rdict['inc'] = inc
    rdict['filt'] = 0.15
